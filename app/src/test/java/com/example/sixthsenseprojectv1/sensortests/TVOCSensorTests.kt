...
//This class tests each method inside of the TVOC sensor object
class TVOCSensorTests {
    @Test
    //tests the getFromDatabase method inside the TVOC sensor object
    fun getFromDatabase(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
        dbHandler.addNewSensorData("1", "TVOC",
    ...
    @Test
    //tests the getWarningFromDatabase method inside the TVOC sensor object
    fun getWarningFromDatabase(){
        ...
        dbHandler.addNewSensorData("1", "TVOC",
            "23", "24", "time1")
        val myTemp = TVOC(1, "TVOC", 90, 23,null)
        val temp = myTemp.getWarningFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals(24,temp)
    }
    @Test
    //tests the checkWarnings method inside the TVOC sensor object
    fun checkWarnings(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAllWarning()
        val myTemp = TVOC(1, "TVOC", 90, 23,null)
        val temp = myTemp.checkWarnings(3000)
        assertEquals(0,temp)
    }

    @Test
    //tests the getHistorical method inside the TVOC sensor object
    fun getHistorical(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
    ...
    @Test
    //tests the getHistoricalWithTime method inside the TVOC sensor object
    ...
    @Test
    //tests the getHistoricalWarning method inside the TVOC sensor object
    ...
    @Test
    //tests the getLastTenMinutes method inside the TVOC sensor object
