...
//This class tests each method inside of the Heart Rate sensor object
class HeartRateSensorTests {
    @Test
    //tests the getFromDatabase method inside the Heart Rate sensor object
    fun getFromDatabase(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
        dbHandler.addNewSensorData("1", "Heart Rate",
            "data1", "warning1", "time1")
        val myTemp = HeartRate(1, "Heart Rate", 90, 0,null)
        val temp = myTemp.getFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals("data1 Bpm",temp)
    }

    @Test
    //tests the getWarningFromDatabase method inside the Heart Rate sensor object
    fun getWarningFromDatabase(){
        ...
        dbHandler.addNewSensorData("1", "Heart Rate",
            "23", "24", "time1")
        val myTemp = HeartRate(1, "Heart Rate", 90, 23,null)
        val temp = myTemp.getWarningFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals(24,temp)
    }
    @Test
    //tests the checkWarnings method inside the Heart Rate sensor object
    fun checkWarnings(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAllWarning()
        val myTemp = HeartRate(1, "Heart Rate", 90, 23,null)
        val temp = myTemp.checkWarnings(150)
        assertEquals(0,temp)
    }

    @Test
    //tests the getHistorical method inside the ECO2 sensor object
    fun getHistorical(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
    ...
    @Test
    //tests the getHistoricalWithTime method inside the Heart Rate sensor object
    ...
    @Test
    //tests the getHistoricalWarning method inside the Heart Rate sensor object
    ...
    @Test
    //tests the getLastTenMinutes method inside the Heart Rate sensor object
