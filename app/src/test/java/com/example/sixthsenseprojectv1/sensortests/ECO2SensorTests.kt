...
//This class tests each method inside of the ECO2 sensor object
class ECO2SensorTests {
    @Test
    //tests the getFromDatabase method inside the ECO2 sensor object
    fun getFromDatabase(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
        dbHandler.addNewSensorData("1", "ECO2",
            "data1", "warning1", "time1")
        val myTemp = ECO2(1, "ECO2", 90, 0,null)
        val temp = myTemp.getFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals("data1 ppm",temp)
    }

    @Test
    //tests the getWarningFromDatabase method inside the ECO2 sensor object
    fun getWarningFromDatabase(){
        ...
        dbHandler.addNewSensorData("1", "ECO2",
            "23", "24", "time1")
        val myTemp = ECO2(1, "ECO2", 90, 23,null)
        val temp = myTemp.getWarningFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals(24,temp)
    }
    @Test
    //tests the checkWarnings method inside the ECO2 sensor object
    fun checkWarnings(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAllWarning()
        val myTemp = ECO2(1, "ECO2", 90, 23,null)
        val temp = myTemp.checkWarnings(1500)
        assertEquals(0,temp)
    }
    ...
    @Test
    //tests the getHistoricalWithTime method inside the ECO2 sensor object
    ...
    @Test
    //tests the getHistoricalWarning method inside the ECO2 sensor object
    ...
    @Test
    //tests the getLastTenMinutes method inside the ECO2 sensor object
