...
//This class tests each method inside of the Pressure sensor object
class PressureSensorTests {
    @Test
    //tests the getFromDatabase method inside the Pressure sensor object
    fun getFromDatabase(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
        dbHandler.addNewSensorData("1", "Pressure",
        ...
        assertEquals("data1 kpa",temp)
    }

    @Test
    //tests the getWarningFromDatabase method inside the Pressure sensor object
    fun getWarningFromDatabase(){
        ...
        dbHandler.addNewSensorData("1", "Pressure",
            "23", "24", "time1")
        val myTemp = Pressure(1, "Pressure", 90, 23,null)
        val temp = myTemp.getWarningFromDatabase(InstrumentationRegistry.getInstrumentation().targetContext,1)
        assertEquals(24,temp)
    }
    @Test
    //tests the checkWarnings method inside the Pressure sensor object
    fun checkWarnings(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAllWarning()
        val myTemp = Pressure(1, "Pressure", 90, 23,null)
        val temp = myTemp.checkWarnings(90)
        assertEquals(0,temp)
    }

    @Test
    //tests the getHistorical method inside the Pressure sensor object
    fun getHistorical(){
        val dbHandler = DBHandler(context =
        InstrumentationRegistry.getInstrumentation().targetContext)
        dbHandler.deleteAll()
    ...
    @Test
    //tests the getHistoricalWithTime method inside the Pressure sensor object
    ...
    @Test
    //tests the getHistoricalWarning method inside the Pressure sensor object
    ...
    @Test
    //tests the getLastTenMinutes method inside the Pressure sensor object
