...
class DBHandler (context: Context?): SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    fun addNewSensorData( userID: String?, sensorName: String?, data: String?,
                          warning: String?, Time: String? ): Long {
    ...
    fun addNewWarningData( userID: String?, sensorName: String?, data: String?,
                           warning: String?, Time: String? ): Long {
    ...
    fun generateLatestData(sensorName: String, usersID: Int): String {
        ...
        if (sensorName == "Temperature"){
            ...
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'TVOC' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "ECO2"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'ECO2' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Luminosity"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Luminosity' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Longitude"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Longitude' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Latitude"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Latitude' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Time"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Time' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Heart Rate"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Heart Rate' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Oxygen Saturation"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Oxygen Saturation' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Body Temperature"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Body Temperature' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Respiratory"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Respiratory' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Systolic"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Systolic' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Accelerometer"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Accelerometer' AND $USERID_COL = $tempUserID"
        }
        else if(sensorName == "Diastolic"){
            tempQ = "SELECT * FROM $TABLE_NAME WHERE $NAME_COL = 'Diastolic' AND $USERID_COL = $tempUserID"
        }
    ...
    fun exportToCSV2(context: Context) {
        ...
        val fileName = "SensorData_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMMdd_HH-mm-ss")) + ".csv"
        val relativeLocation = Environment.DIRECTORY_DOCUMENTS + File.separator + "SixthSenseImports" // Directory path where you want to save the file
        ...
        uri?.let { uri ->
            try {
                ...
                Toast.makeText(context, "File Exported Successfully in $uri", Toast.LENGTH_SHORT).show()
            } catch (e: IOException) {
                Toast.makeText(context, "Error Occurred while exporting File: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    fun exportWarningsCsv(context: Context,sensorName: String?)
    {
        ...
        val relativeLocation = Environment.DIRECTORY_DOCUMENTS + File.separator + "MyApp" // Directory path where you want to save the file
        val resolver = context.contentResolver
        val contentValues = ContentValues().apply {
            put(MediaStore.MediaColumns.DISPLAY_NAME, fileName)
            put(MediaStore.MediaColumns.MIME_TYPE, "text/csv")
        ...
        uri?.let { uri ->
            try {
                ...
                Toast.makeText(context, "Warnings File Exported Successfully in $uri", Toast.LENGTH_SHORT).show()
            } catch (e: IOException) {
                Toast.makeText(context, "Error Occurred while exporting Warnings File: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    fun createXlFile(context: Context) {
        ...
        val fileName = folderName + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMMdd_HH-mm-ss")) + ".xls"
        val path: String =
