...
class Diastolic(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try {
            ...
            latestData = dbHandler.readLatestPerSensor("Diastolic", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("Diastolic", personID).size == 0) {
                return returnData;
            }

            returnData = latestData.get(0).data + " hg"
        } catch (e: Exception) {
            //handle exception
        }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Diastolic", personID).toInt()
            if (dbHandler.readLatestWarningPerSensor("Diastolic", personID).isEmpty()) {
                return 0;
            }
        } catch (e: Exception) {
            //handle exception
    ...
    override fun getHistorical(context: Context, personID: Int): ArrayList<String> {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readAllPerSensor("Diastolic", personID)

            val size = latestData.size - 1
            for (i in 0..size) {
                val m = latestData[i]
                if (m.sensorName == "Diastolic") {
    ...
    override fun getHistoricalWithTime(
        ...
        try {
            ...
            val latestData = dbHandler.readAllPerSensor("Diastolic", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Diastolic", personID)
