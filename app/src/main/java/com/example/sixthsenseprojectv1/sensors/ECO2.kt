...
//class for eCO2 sensor
class ECO2(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try{
            ...
            latestData = dbHandler.readLatestPerSensor("ECO2", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("ECO2", personID).size == 0)
            {
                return returnData;
            }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("ECO2",personID).toInt()
            if(dbHandler.readLatestWarningPerSensor("ECO2",personID).isEmpty())
            {
                return 0;
            }
        }
    ...
    override fun getHistorical(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readAllPerSensor("ECO2", personID)

            val size = latestData.size - 1
            for (i in 0..size){
                val m = latestData[i]
                if (m.sensorName == "ECO2")
                {
    ...
    override fun getHistoricalWithTime(context: Context, personID: Int): MutableMap<String, String> {
        ...
        try{
            ...
            val latestData = dbHandler.readAllPerSensor("ECO2", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("ECO2", personID)
    ...
    override fun dummyProvider(personID: Int, context: Context) {
        ...
        val eco2 = (400..8192).random()
        checkWarnings(eco2)
        addToDatabase(personID, "ECO2", eco2.toDouble(), tempWarning, current, context)
    }
