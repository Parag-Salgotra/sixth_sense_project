...
class Accelerometer(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try {
            ...
            latestData = dbHandler.readLatestPerSensor("Accelerometer", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("Accelerometer", personID).size == 0) {
                return returnData;
            }

            returnData = latestData.get(0).data + " g"
        } catch (e: Exception) {
            //handle exception
        }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Accelerometer", personID).toInt()
            if (dbHandler.readLatestWarningPerSensor("Accelerometer", personID).isEmpty()) {
                return 0;
            }
        } catch (e: Exception) {
            //handle exception
    ...
    override fun getHistorical(context: Context, personID: Int): ArrayList<String> {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readAllPerSensor("Accelerometer", personID)

            val size = latestData.size - 1
            for (i in 0..size) {
                val m = latestData[i]
                if (m.sensorName == "Accelerometer") {
    ...
    override fun getHistoricalWithTime(
        ...
        try {
            ...
            val latestData = dbHandler.readAllPerSensor("Accelerometer", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try {
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Accelerometer", personID)
