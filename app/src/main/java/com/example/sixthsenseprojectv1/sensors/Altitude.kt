...
class Altitude(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try{
            ...
            latestData = dbHandler.readLatestPerSensor("Altitude", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("Altitude", personID).size == 0)
            {
                return returnData;
            }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Altitude",personID).toInt()
            if(dbHandler.readLatestWarningPerSensor("Altitude",personID).isEmpty())
            {
                return 0;
            }
        }
    ...
    override fun getHistoricalWithTime(context: Context, personID: Int): MutableMap<String, String> {
        ...
        try{
            ...
            val latestData = dbHandler.readAllPerSensor("Altitude", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Altitude", personID)
    ...
    override fun dummyProvider(personID: Int, context: Context) {
        ...
        val altitude = (0..10).random()
        checkWarnings(altitude)
        addToDatabase(personID, "Altitude", altitude.toDouble(), tempWarning, current, context)
    }
