...
abstract class Sensors(id: Int, name: String, data: Any, warning: Int, time: LocalDateTime?) {
    ...
    fun addToDatabase(id: Int, name: String, data: Double, warning: Int, time: LocalDateTime?, context: Context) {
        val dbHandler = DBHandler(context)
        //val index = sensorNames.getIndexOf(name)
        dbHandler.addNewSensorData(id.toString(),name,data.toString(),warning.toString(),time.toString())
    ...
    fun addWarningData(id: Int, name: String, data: Double, warning: Int, time: LocalDateTime?, context: Context){
        val dbHandler = DBHandler(context)
        val index = sensorNames.indexOf(name)
        dbHandler.addNewWarningData(id.toString(),name,data.toString(),warning.toString(),time.toString())
        if (warning == 0) {
