...
class Humidity(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try{
            ...
            latestData = dbHandler.readLatestPerSensor("Humidity", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("Humidity", personID).size == 0)
            {
                return returnData;
            }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Humidity",personID).toInt()
            if(dbHandler.readLatestWarningPerSensor("Humidity",personID).isEmpty())
            {
                return 0;
            }
        }
    ...
    override fun getHistorical(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readAllPerSensor("Humidity", personID)

            val size = latestData.size - 1
            for (i in 0..size){
                val m = latestData[i]
                if (m.sensorName == "Humidity")
                {
    ...
    override fun getHistoricalWithTime(context: Context, personID: Int): MutableMap<String, String> {
        ...
        try{
            ...
            val latestData = dbHandler.readAllPerSensor("Humidity", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Humidity", personID)
    ...
    override fun dummyProvider(personID: Int, context: Context) {
        ...
        val humidity = (1..100).random()
        checkWarnings(humidity)
        addToDatabase(personID, "Humidity", humidity.toDouble(), tempWarning, current, context)
    }
