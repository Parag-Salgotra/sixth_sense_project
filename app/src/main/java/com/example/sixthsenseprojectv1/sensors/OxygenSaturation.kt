...
//class for oxygen saturation sensor
class OxygenSaturation(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getFromDatabase(context: Context, personID: Int): String {
        ...
        try{
            ...
            latestData = dbHandler.readLatestPerSensor("Oxygen Saturation", personID)
            //If condition to return empty string if the table has size 0
            if (dbHandler.readLatestPerSensor("Oxygen Saturation", personID).size == 0)
            {
                return returnData;
            }
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Oxygen Saturation",personID).toInt()
            if(dbHandler.readLatestWarningPerSensor("Oxygen Saturation",personID).isEmpty())
            {
                return 0;
            }
        }
    ...
    override fun getHistorical(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readAllPerSensor("Oxygen Saturation", personID)

            val size = latestData.size - 1
            for (i in 0..size){
                val m = latestData[i]
                if (m.sensorName == "Oxygen Saturation")
                {
    ...
    override fun getHistoricalWithTime(context: Context, personID: Int): MutableMap<String, String> {
        ...
        try{
            ...
            val latestData = dbHandler.readAllPerSensor("Oxygen Saturation", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Oxygen Saturation", personID)
    ...
    override fun dummyProvider(personID: Int, context: Context) {
        ...
        val oxygenSaturation = (0..100).random()
        checkWarnings(oxygenSaturation)
        addToDatabase(personID, "Oxygen Saturation", oxygenSaturation.toDouble(), tempWarning, current, context)
    }
