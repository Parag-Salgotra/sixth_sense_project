...
class Temperature(id: Int, name: String, data: Int, warning: Int, time: LocalDateTime?) : Sensors(id, name, data, warning, time) {
    ...
    override fun getWarningFromDatabase(context: Context, personID: Int): Int {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            returnData = dbHandler.readLatestWarningPerSensor("Temperature",personID).toInt()
            if(dbHandler.readLatestWarningPerSensor("Temperature",personID).isEmpty())
            {
                return 0;
            }
        }
    ...
    override fun getHistoricalWithTime(context: Context, personID: Int): MutableMap<String, String> {
        ...
        try{
            ...
            val latestData = dbHandler.readAllPerSensor("Temperature", personID)
    ...
    override fun getLastTenMinutes(context: Context, personID: Int): ArrayList<String> {
        ...
        try{
            val dbHandler: DBHandler = DBHandler(context)
            var latestData: ArrayList<SensorModelDB> = ArrayList<SensorModelDB>()
            latestData = dbHandler.readTenMinutes("Temperature", personID)
    ...
    override fun dummyProvider(personID: Int, context: Context) {
        ...
        addToDatabase(personID, "Temperature", temperature.toDouble(), tempWarning, current, context)
    }
